...
import (
	...
	"timtube/domain"
	user2 "timtube/io/user/user"
	"timtube/io/user/user-account"
)
...
func loginPostHandler(app *config.Env) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		userData, err := user2.ReadUser(email)
		if err != nil {
			fmt.Println(err, " error login user")
		}
		if userData.Name != "" && userData.Surname != "" {
			util.CreateSession("name", userData.Name, app, r)
			util.CreateSession("surname", userData.Surname, app, r)
			util.CreateSession("role", userData.RoleId, app, r)
